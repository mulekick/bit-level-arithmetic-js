...
const
    // ====================================================
    // Declarations
    {chalk, table, DataViewAugmented, le, NUMBER_16_BITS, NUMBER_32_BITS, addFloatingPoint, subtractFloatingPoint, multiplyFloatingPoint, longDivideFloatingPoint, dec2base, asciitbl, deepcopy, output, add, sub, mul, div} = require(`./lib.js`),
    ...
    formatresults = (nres, a, col, view, nbits, f, getint, getfloat) => {
        ...
        // format output
        // ------------------------------------------------
        a[row++][col] = `${ nbits } BITS FLOATING POINT`;
        a[row++][col] = `FLOATING POINT BINARY :`;
...
try {
    ...
    for (let i = 0; i < itc.length; i++) {
        ...
        tbl
            .setHeading([ `${ nbits } BITS FLOATING POINT`, `BINARY`, `RATIONAL` ])
            .addRowMatrix([
